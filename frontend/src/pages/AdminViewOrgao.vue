  ...
  <section class="citizen-page">
    ...
    <header class="topbar">
      ...
      <nav class="topbar__nav">
        ...
        <button class="nav-link" type="button" @click="$router.push('/admin/tipos')">
          Gerenciar Tipos
        </button>
      </nav>
    ...
    <section class="hero-panel">
      ...
      <h1>Gerenciamento de órgãos</h1>
      <p>
        Consulte os órgãos cadastrados, busque por ID ou nome e realize alterações ou exclusões diretamente pela tabela.
      </p>
    </section>
      ...
      <div class="toolbar-panel">
        <div class="search-container">
          <InputText
              id="search-orgao"
              :modelValue="busca"
              placeholder="Buscar por ID ou nome do órgão"
        ...
        <div class="toolbar-right">
          <button class="primary-action" type="button" @click="abrirNovoOrgao">
            Novo órgão
          </button>
        </div>
      </div>
    ...
    <section class="reports-shell">
      <div class="section-heading">
        ...
        <span class="section-caption">
          Visualização administrativa dos órgãos registrados
        </span>
      </div>
      ...
      <div v-if="listaFiltrada.length > 0" class="table-shell">
        <table class="orgao-table">
          ...
          <tbody>
          <tr v-for="orgao in listaFiltrada" :key="orgao.id">
            <td class="td-id">#{{ orgao.id }}</td>
            <td class="td-nome">{{ orgao.nome }}</td>
            <td class="td-actions">
              <button
      ...
      <div v-else class="feedback-hint">
        Nenhum órgão encontrado. Ajuste a busca ou cadastre um novo órgão.
      </div>
    </section>

    <Form
        :show="showForm"
        :title="modoEdicao ? 'Editar órgão' : 'Novo órgão'"
        :schema="formSchema"
        :initial-data="orgaoSelecionado"
        @close="fecharForm"
        @confirm="salvarOrgao"
    />
  </section>
  ...
  methods:
      {
        ...
        aplicarFiltros()
        {
          let lista = [...this.listaOrgaos];

          if (this.busca?.trim())
          {
            const termo = this.busca.toLowerCase().trim();

            lista = lista.filter(orgao =>
                String(orgao?.id || '').includes(termo) ||
                orgao?.nome?.toLowerCase().includes(termo)
            );
          }
        ...
        async buscarOrgaos()
        {
          await api.get("/apis/admin/all-institutions")
              .then(response =>
              {
                if (response.status === 200)
                {
                  this.listaOrgaos = response.data;
                  this.listaOrgaos.sort((a, b) => a.id - b.id);
                  this.aplicarFiltros();
                }
                else
                {
        ...
        async salvarOrgao(formDados)
        {
          try
          {
            ...
            if (this.modoEdicao && this.orgaoSelecionado?.id)
            {
              const payload =
                  {
                    nome: formDados.nome,
              ...
              response = await api.put(`/apis/admin/update-institution`, payload);
            }
            else
            {
              const payload =
              ...
              response = await api.post("/apis/admin/add-institution", payload);
            }

            if (response.status === 200 || response.status === 201)
            {
              this.$toast.success(this.modoEdicao ? "Órgão atualizado com sucesso!" : "Órgão cadastrado com sucesso!");
              this.fecharForm();
          ...
          {
            this.$toast.error(this.modoEdicao ? "Erro ao atualizar o órgão." : "Erro ao cadastrar o órgão.");
          }
        },

        confirmarDelecao(orgao)
        {
          api.delete(`/apis/admin/delete-institution/${orgao.id}`)
              .then(response =>
              {
                if (response.status === 200)
                {
                  this.$toast.success(`O órgão "${orgao.nome}" foi deletado com sucesso!`);
                  this.buscarOrgaos();
                }
              })
              .catch(() =>
              {
                this.$toast.error("Ocorreu um erro ao deletar o órgão.");
              });
        },
